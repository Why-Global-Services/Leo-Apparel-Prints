...
import { motion } from "framer-motion";
import { Trash2, Plus, Minus, ShoppingBag, ArrowLeft, Sparkles } from "lucide-react";
import { fetchCart, removeFromCart, editCartItem } from "@/features/cart/cartThunks";
...
export default function CartPage() {
  ...
  const router = useRouter();
  const { user } = useSelector((state) => state.auth);
  const { items: cartItems, loading, isGuest } = useSelector((state) => state.cart);
  const [isAuthModalOpen, setIsAuthModalOpen] = useState(false);
  const [authModalMode, setAuthModalMode] = useState("login");
  
  const cartItemCount = cartItems?.reduce((total, item) => total + (item.quantity || 0), 0) || 0;
  const total = cartItems?.reduce((sum, item) => sum + ((item.basePrice || item.price) * item.quantity), 0) || 0;

  useEffect(() => {
    dispatch(fetchCart());
  }, [dispatch]);

  const handleUpdateQuantity = async (itemId, newQuantity, itemSize) => {
    if (newQuantity < 1) {
    ...
    try {
      // For guest users, we need to use the item's id directly
      // For logged-in users, use customizationId
      await dispatch(editCartItem({ 
        customizationId: itemId, 
    ...
    return (
      <>
        <div className="min-h-screen bg-gray-50">
          ...
          <div className="sticky top-0 z-20 bg-white shadow-sm mt-2 lg:hidden">
            <div className="flex items-center justify-between px-4 py-2.5">
              <button onClick={handleGoBack} className="w-6 h-6 rounded-full bg-gradient-to-r from-accent to-accent-dark flex items-center justify-center shadow-sm active:scale-95 transition-all duration-200">
                <ArrowLeft size={12} className="text-white" />
              </button>
              <h1 className="text-base font-bold italic text-primary-blue-dark">Cart <span className="text-primary">({cartItemCount})</span></h1>
              <Link href="/products" className="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center active:bg-gray-200 transition-all duration-200">
                <ShoppingBag size={16} className="text-gray-700" />
              </Link>
            </div>
          </div>
          ...
          <div className="hidden lg:block max-w-7xl mx-auto px-7 py-10">
            <div className="mb-8 flex justify-between items-center">
              <div>
                <h1 className="text-4xl text-black font-bold italic uppercase tracking-tighter font-primary">Shopping <span className="text-primary">Cart</span></h1>
                <p className="text-gray-500 font-secondary">{cartItemCount} items in your cart</p>
              </div>
              <Link href="/products" className="inline-flex items-center gap-2 group cursor-pointer">
                <span className="text-gray-700 font-medium group-hover:text-primary-blue transition-all duration-300">Continue Shopping</span>
                <div className="w-8 h-8 rounded-full bg-gradient-to-r from-accent to-accent-dark flex items-center justify-center shadow-md group-hover:shadow-lg transition-all duration-300 group-hover:scale-110">
                  <ArrowLeft size={18} className="text-white rotate-180" />
                </div>
              </Link>
            ...
            <div className="flex flex-col lg:flex-row gap-5 lg:gap-8">
                          ...
                          <div className="p-3"> 
                            <div className="flex gap-3">
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex justify-between items-start gap-1">
                                  <div className="flex-1">
                                    <h3 className="font-semibold text-gray-800 font-primary text-sm line-clamp-1">{item.productName || item.name}</h3>
                                    <p className="text-xs text-gray-500">{item.size && `Size: ${item.size}`}</p>
                                  </div>
                                  <button onClick={() => removeItem(itemId)} className="p-1 -mt-1 -mr-1 text-gray-400 active:text-red-500 transition-colors">
                                    <Trash2 size={14} />
                                  </button>
                                </div>
                                <div className="flex items-center justify-between mt-2">
                                  <span className="text-sm font-bold text-primary-blue">₹{item.basePrice || item.price}</span>
                                  <div className="flex items-center gap-2">
                                    <button 
                                      onClick={() => handleUpdateQuantity(itemId, (item.quantity || 1) - 1, item.size)} 
                                      className="w-5 h-5 rounded-md bg-primary-blue text-white active:bg-primary-blue-dark transition-all duration-200 flex items-center justify-center"
                                    >
                                      <Minus size={12} />
                                    </button>
                                    <span className="font-semibold text-gray-800 w-6 text-center text-sm">{item.quantity || 1}</span>
                                    <button 
                                      onClick={() => handleUpdateQuantity(itemId, (item.quantity || 1) + 1, item.size)} 
                                      className="w-5 h-5 rounded-md bg-primary-blue text-white active:bg-primary-blue-dark transition-all duration-200 flex items-center justify-center"
                                    >
                                  ...
                                  <span className="text-xs font-semibold text-gray-700">₹{((item.basePrice || item.price) * (item.quantity || 1)).toFixed(0)}</span>
                                </div>
                                <button onClick={() => handleCustomize(item.productId)} className="w-full mt-2 flex items-center justify-center gap-1.5 py-1.5 btn btn-gradient btn-sm btn-shine text-xs rounded-md">
                                  <Sparkles size={10} className="text-yellow-200" /> Customize
                                </button>
                              </div>
                            </div>
                          </div>
                        ...
                        <div className="hidden lg:grid lg:grid-cols-12 gap-2 p-4 border-b last:border-b-0 hover:bg-gray-50/30 transition-colors duration-200 items-center">
                          <div className="col-span-4">
                            <div className="flex gap-3 items-center">
                              ...
                              <div className="flex-1 min-w-0">
                                <h3 className="font-semibold text-gray-800 font-primary text-sm truncate">{item.productName || item.name}</h3>
                                {item.size && <p className="text-xs text-gray-400">Size: {item.size}</p>}
                              </div>
                            </div>
                          </div>
                          <div className="col-span-2 flex items-center justify-center">
                            <button onClick={() => handleCustomize(item.productId)} className="inline-flex items-center gap-1 px-3 py-1.5 btn btn-gradient btn-sm btn-shine text-xs whitespace-nowrap">
                              <Sparkles size={11} className="text-yellow-200" /> Customize
                            </button>
                          </div>
                          <div className="col-span-2 text-center">
                            <span className="font-semibold text-gray-800 text-sm">₹{item.basePrice || item.price}</span>
                          </div>
                          <div className="col-span-2">
                            <div className="flex items-center justify-center gap-2">
                              <button 
                              ...
                              <span className="font-semibold text-gray-800 w-7 text-center text-sm">{item.quantity || 1}</span>
                              <button 
                                onClick={() => handleUpdateQuantity(itemId, (item.quantity || 1) + 1, item.size)} 
                                className="w-7 h-7 rounded-lg bg-primary-blue text-white hover:bg-primary-blue-dark transition-all duration-300 flex items-center justify-center"
                              >
                          ...
                          <div className="col-span-1 text-center">
                            <span className="font-bold text-primary-blue text-sm">₹{((item.basePrice || item.price) * (item.quantity || 1)).toFixed(2)}</span>
                          </div>
                          <div className="col-span-1 text-center">
                            <button onClick={() => removeItem(itemId)} className="p-1.5 text-gray-400 hover:text-red-500 transition-all duration-300">
                              <Trash2 size={15} />
                            </button>
                          </div>
                        </div>
              ...
              <div className="hidden lg:block lg:w-96 flex-shrink-0">
                <div className="bg-white rounded-2xl shadow-sm p-6 sticky top-24">
                  ...
                  <button onClick={handleCheckout} className="w-full btn btn-gradient btn-lg btn-shine mt-6">
                    Proceed to Checkout
                    <svg className="w-5 h-5" viewBox="0 0 14 14" fill="none">
                      <path d="M2 7h10M8 3.5L11.5 7 8 10.5" stroke="white" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                  </button>
                  {!user && <p className="text-xs text-center text-gray-500 mt-3">Login to complete your purchase</p>}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-lg z-30">
            <div className="px-4 py-3">
              <div className="flex justify-between items-center mb-2">
                ...
                <button onClick={handleCheckout} className="flex-1 ml-4 btn btn-gradient btn-md btn-shine py-2.5 text-sm font-semibold">
                  Checkout
                  <svg className="w-3.5 h-3.5 ml-1.5" viewBox="0 0 14 14" fill="none">
                    <path d="M2 7h10M8 3.5L11.5 7 8 10.5" stroke="white" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
                  </svg>
              ...
              {!user && <p className="text-xs text-center text-gray-500 mt-1">Login to complete your purchase</p>}
            </div>
          </div>
        </div>

        {/* Auth Modal */}
        <AuthModal isOpen={isAuthModalOpen} onClose={closeAuthModal} defaultMode={authModalMode} onSuccess={handleAuthSuccess} />
      </>
    );
  }
    ...
    <div className="min-h-screen bg-gray-50 py-12">
      <div className="max-w-4xl mx-auto px-4">
        <div className="bg-white rounded-2xl shadow-sm p-8 text-center">
          ...
          <h1 className="text-2xl font-bold text-gray-800 mb-2 font-primary">Your Cart is Empty</h1>
          <p className="text-gray-500 mb-6 font-secondary">Looks like you haven't added any items to your cart yet.</p>
          <Link href="/products" className="btn btn-gradient btn-md inline-flex">Continue Shopping</Link>
        </div>
      </div>
    </div>
  );
