...
//   // Logos — blobUrl for display, File ref for upload
//   const [clubLogo,    setClubLogo]    = useState(null); // blob URL or existing CDN URL
//   const [sponsorLogo, setSponsorLogo] = useState(null);
//   const clubLogoFileRef    = useRef(null); // raw File object
...
//   // Base product images (may be swapped by pattern)
...
//     setFrontImage(pattern.frontPattern);
//     setBackImage(pattern.backPattern);
//   };

//   // ── Logo upload — stores File in ref, blob URL in state ───────────────────
//   // FIX: pass both setter (for display URL) AND fileRef (for upload)
...
//     try {
//       // Step 1: save customization (sequential — not Promise.all)
//       const customizationRes = await saveCustomizationAPI({
//         productId:       product?._id || product?.id,
//         customizationId: customizationId || '',
//         customization: [
//           { zoneKey, fieldName: 'jerseyColor',  value: String(jerseyColor)  },
//           { zoneKey, fieldName: 'sleeveColor',  value: String(sleeveColor)  },
//           { zoneKey, fieldName: 'collarColor',  value: String(collarColor)  },
//           { zoneKey, fieldName: 'playerName',   value: showName   ? String(playerName)   : '' },
//           { zoneKey, fieldName: 'playerNumber', value: showNumber ? String(playerNumber) : '' },
//           { zoneKey, fieldName: 'fabric',       value: String(fabric)       },
//           { zoneKey, fieldName: 'nameFont',     value: String(nameFont)     },
//           { zoneKey, fieldName: 'nameColor',    value: String(nameColor)    },
//           { zoneKey, fieldName: 'numberFont',   value: String(numberFont)   },
//           { zoneKey, fieldName: 'numberColor',  value: String(numberColor)  },
//           { zoneKey, fieldName: 'nameStyle',    value: String(nameStyle)    },
//           { zoneKey, fieldName: 'textEffect',   value: String(textEffect)   },
//           {
//   zoneKey,
//   fieldName: 'patternId',
...
// {
//   zoneKey,
//   fieldName: 'patternFront',
//   value: selectedPattern?.frontPattern || ''
// },

// {
//   zoneKey,
//   fieldName: 'patternBack',
//   value: selectedPattern?.backPattern || ''
// },
//         ],
//         // raw File objects — null if user hasn't picked a new file
//         clubLogo:    clubLogoFileRef.current,
//         sponsorLogo: sponsorLogoFileRef.current,
        
//       });

//       // Step 2: use the returned ID to add to cart
...
//   // FIX: UploadSlot now correctly wires the file input to handleLogoUpload
...
//       {/* FIX: onChange uses handleLogoUpload with both setter AND fileRef */}
...
//               <button
//                 onClick={() => { setSelectedPattern(null); setFrontImage(product?.viewImages?.front || product?.images?.[0]); setBackImage(product?.viewImages?.back || product?.images?.[1]); }}
...
//                 <button key={pattern._id} onClick={() => handlePatternSelect(pattern)} style={{ border: selectedPattern?._id === pattern._id ? '2px solid #003E9B' : '1px solid #E2E8F0', borderRadius: 10, padding: 6, background: '#fff', cursor: 'pointer' }}>
//                   <img src={pattern.thumbnail} alt={pattern.name} style={{ width: '100%', borderRadius: 8 }} />
...
//           {/* FIX: pass fileRef so raw File is stored for upload */}
//           <UploadSlot
//             label="Club Badge"
...
//           <div style={{ fontSize: 10, color: '#1D4ED8', fontWeight: 600 }}>✦ Name &amp; number update live on the 2D preview. Switch to <b>3D View</b> for full model preview.</div>
//         </div>
//       </>
//     );
...
//             />
//             {/* Canvas2DOverlay draws logos + name + number on top of the image */}
//             <Canvas2DOverlay
