...
// ── UI ATOMS ──────────────────────────────────────────────────────────────
const Section = ({ title, badge, open, onToggle, children }) => (
  <div style={{ background: '#fff', borderRadius: 12, marginBottom: 10, overflow: 'hidden', border: `1px solid ${open ? 'rgba(0,62,155,0.2)' : '#E8ECF0'}`, boxShadow: open ? '0 2px 14px rgba(0,62,155,0.07)' : 'none', width: '100%', boxSizing: 'border-box' }}>
    <button onClick={onToggle} style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 14px', border: 'none', cursor: 'pointer', background: open ? 'rgba(0,62,155,0.04)' : '#FAFAFA', boxSizing: 'border-box' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <span style={{ fontSize: 11, fontWeight: 700, color: open ? '#003E9B' : '#334155' }}>{title}</span>
        {badge && <span style={{ fontSize: 9, padding: '2px 8px', borderRadius: 12, background: 'rgba(0,62,155,0.10)', color: '#003E9B', fontWeight: 700 }}>{badge}</span>}
      </div>
    ...
    {open && <div style={{ padding: '12px 10px', display: 'flex', flexDirection: 'column', gap: 12, width: '100%', boxSizing: 'border-box', overflowX: 'hidden' }}>{children}</div>}
  </div>
);
...
const NameStyleSelect = ({ selected, onSelect, sampleText, sampleFontId }) => {
  const fo = getFontObj(sampleFontId), displayText = (sampleText || 'NAME').slice(0, 8);
  const curvedFontSize = Math.max(8, Math.min(13, 60 / Math.max(1, displayText.length)));
  const straightFontSize = Math.max(8, Math.min(15, 65 / Math.max(1, displayText.length)));
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: 6, width: '100%', boxSizing: 'border-box' }}>
      {NAME_STYLES.map(ns => {
        const isSel = selected === ns.id, col = isSel ? '#003E9B' : '#334155';
        return (
          <button
            ...
            style={{
              ...
              borderRadius: 10,
              cursor: 'pointer',
              border: `2px solid ${isSel ? '#003E9B' : '#E2E8F0'}`,
              background: isSel ? 'rgba(0,62,155,0.07)' : '#F8FAFC',
              display: 'flex',
          ...
          >
            ...
            >
              {ns.id === 'none' && (
                <>
                  <circle cx="42" cy="19" r="13" fill="none" stroke={isSel ? '#003E9B' : '#CBD5E1'} strokeWidth="2" />
                  <line x1="31" y1="28" x2="53" y2="10" stroke={isSel ? '#003E9B' : '#CBD5E1'} strokeWidth="2.5" strokeLinecap="round" />
                </>
              )}
              {ns.id === 'straight' && (
                <text x="42" y="24" textAnchor="middle" fontFamily={`${fo.canvasFont}, sans-serif`} fontWeight={fo.fontWeight} fontSize={straightFontSize} fill={col}>
                  {displayText}
                </text>
              )}
              {ns.id === 'curved' && (
                <>
                  ...
                  <text fontFamily={`${fo.canvasFont}, sans-serif`} fontWeight={fo.fontWeight} fontSize={curvedFontSize} fill={col}>
            ...
            <span style={{ fontSize: 9, fontWeight: 700, color: isSel ? '#003E9B' : '#64748B', whiteSpace: 'nowrap' }}>
              {ns.label}
            </span>
          </button>
        );
    ...
    {FONT_STYLES.map(font => (
      <button
        ...
        style={{
          ...
          borderRadius: 10,
          cursor: 'pointer',
          border: `2px solid ${selected === font.id ? '#003E9B' : '#E2E8F0'}`,
          background: selected === font.id ? 'rgba(0,62,155,0.07)' : '#F8FAFC',
          display: 'flex',
          flexDirection: 'column',
          alignItems: 'center',
          justifyContent: 'center',
      ...
      >
        <span style={{ fontFamily: font.fontFamily, fontWeight: font.fontWeight, fontSize: 12, color: selected === font.id ? '#003E9B' : '#1E293B', letterSpacing: 0.5, lineHeight: 1.2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: '100%' }}>PLAYER</span>
        <span style={{ fontSize: 8, fontWeight: 700, color: selected === font.id ? '#003E9B' : '#94A3B8', letterSpacing: 0.6, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: '100%' }}>{font.label}</span>
      </button>
    ))}
  </div>
);
...
const TextEffectSelect = ({ selected, onSelect }) => (
  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: 6, width: '100%', boxSizing: 'border-box' }}>
    {['none', 'outline', 'shadow'].map(ef => (
      <button key={ef} onClick={() => onSelect(ef)} style={{ width: '100%', minWidth: 0, padding: '8px 4px', borderRadius: 9, fontSize: 10, fontWeight: 700, background: selected === ef ? '#003E9B' : '#F3F4F6', color: selected === ef ? '#fff' : '#374151', border: `1.5px solid ${selected === ef ? '#003E9B' : '#E2E8F0'}`, cursor: 'pointer', textTransform: 'capitalize', boxSizing: 'border-box' }}>{ef}</button>
    ))}
  </div>
);

// =====================================================================
// CUSTOM COTTON TEE — interactive design/logo layer + stage
// These two components are new. They are intentionally NOT built on
// top of Canvas2DOverlay: Canvas2DOverlay is a read-only renderer driven
// by admin-defined, fixed printZones (from product data). Here the user
// needs to interactively drag/scale/rotate their own uploaded image, so
// a small self-contained DOM-based layer (no canvas re-draw loop needed)
// is simpler and avoids bending Canvas2DOverlay's zone-based contract.
// =====================================================================
// =============================
// CUSTOM COTTON TEE / UPDATE
// Design auto-fits the print area on upload (sized as a fixed % of the
// A4 box via `sizePct`) and can additionally be zoomed in/out via
// +/- buttons (transform.scale), on top of position (x/y) drag.
// Rotation remains fixed/unused per the earlier request — only zoom
// and move are exposed to the user.
//
// The layer is bounded on BOTH width and height (not just width), with
// the <img> using object-fit: contain, so a tall/wide upload can't
// balloon past the shirt — zooming scales that whole bounded box up or
// down from its center.
// =============================
function CottonTransformableLayer({ src, transform, onChange, label, containerRef, sizePct = 55 }) {
  const handlePointerDown = (e) => {
    ...
    const handleMove = (moveEvent) => {
      ...
      const nx = Math.max(0, Math.min(100, startTransform.x + dxPct));
      const ny = Math.max(0, Math.min(100, startTransform.y + dyPct));
      onChange(prev => ({ ...prev, x: nx, y: ny }));
    };
    const handleUp = () => {
  ...
  return (
    <div
      ...
      style={{
        ...
        left: `${transform.x}%`,
        top: `${transform.y}%`,
        transform: `translate(-50%, -50%) scale(${transform.scale || 1})`,
        width: `${sizePct}%`,
        height: `${sizePct}%`,
        cursor: 'move',
        touchAction: 'none',
...
function CottonDesignStage({
  baseImage, view,
  designPreview, designTransform, onDesignChange,
  logoPreview, logoTransform, onLogoChange,
}) {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      />
      {/*
        CUSTOM COTTON TEE / UPDATE: the A4 print-area border + label are
        no longer shown to the user. This div still exists as an
        invisible anchor so drag math and % positioning have a stable
        reference box — it just renders with no visible border now.
      */}
      <div
        ref={a4Ref}
        style={{ position: 'absolute', left: '25%', top: '15%', width: '50%', height: '70%' }}
      >
        <CottonTransformableLayer src={designPreview} transform={designTransform} onChange={onDesignChange} label="design" containerRef={a4Ref} sizePct={50} />
        {view === 'front' && (
          <CottonTransformableLayer src={logoPreview} transform={logoTransform} onChange={onLogoChange} label="logo" containerRef={a4Ref} sizePct={24} />
        )}
      </div>
    </div>
  );
}
// ── END CUSTOM COTTON TEE atoms ─────────────────────────────────────
...
export default function JerseyCustomizer({ product }) {
  ...
  // =============================
  const isCustomCottonTees = product?.customizationType === 'CUSTOM_COTTON_TEES';
  const isCottonOurDesign = isCustomCottonTees && product?.cottonTeeType === 'OUR_DESIGN';
  const isCottonUploadDesign = isCustomCottonTees && product?.cottonTeeType === 'UPLOAD_DESIGN';
  // ── END detection ────────────────────────────────────────────────
  ...
  // KEY FIX: if the product has a GLB file, show all tabs regardless of
  // printZones — logos/name/number are rendered directly on the 3D model.
  // CUSTOM COTTON TEE: never show the 3D viewer for cotton tee products,
  // regardless of whether product.glbUrl happens to be set.
  const hasGlb = !!product?.glbUrl && !isCustomCottonTees;
  ...
  const visibleTabs = isCottonOurDesign
    ? COTTON_OUR_DESIGN_TABS
    : isCottonUploadDesign
      ? COTTON_UPLOAD_TABS
      : (isCustomizable || hasGlb)
        ? MAIN_TABS_FULL
        : MAIN_TABS_FULL.filter(t => t.id !== 'logos');
  ...
  // =============================
  // CUSTOM COTTON TEE — front/back image resolution with fallback
  // =============================
  const [frontImage, setFrontImage] = useState(
    product?.cottonTee?.frontImage || product?.viewImages?.front || product?.images?.[0]
  );
  const [backImage, setBackImage] = useState(
    product?.cottonTee?.backImage || product?.viewImages?.back || product?.images?.[1]
  );
  // ── END front/back image ─────────────────────────────────────────
  ...
  // =============================
  // CUSTOM COTTON TEE — upload design state (UPLOAD_DESIGN flow only)
  // =============================
  ...
  // Raw File objects are held in refs (same pattern as clubLogoFileRef /
  // sponsorLogoFileRef above) so they're available synchronously for
  // validation and upload without needing extra state plumbing.
  const frontDesignFileRef = useRef(null);
  const backDesignFileRef = useRef(null);
  const cottonLogoFileRef = useRef(null);

  // Design auto-fits the A4 area on upload — scale/rotation are fixed
  // (kept in the object only so the saved shape matches backend
  // validation); only x/y (drag position) is user-adjustable.
  const [frontDesignTransform, setFrontDesignTransform] = useState({ x: 50, y: 50, scale: 1, rotation: 0 });
  const [backDesignTransform, setBackDesignTransform] = useState({ x: 50, y: 50, scale: 1, rotation: 0 });
  const [logoTransform, setLogoTransform] = useState({ x: 20, y: 20, scale: 1, rotation: 0 });

  // Simple size + quantity selector used by BOTH Cotton Tee flows
  // (OUR_DESIGN and UPLOAD_DESIGN) — no name/number/sleeve needed here.
  const [cottonSizes, setCottonSizes] = useState([{ id: Date.now().toString(), size: 'M', quantity: 1 }]);
  const cartSizes = searchParams.get('sizes');

  useEffect(() => {
    if (!isCottonOurDesign || !cartSizes) return;

    try {
      const sizesFromCart = JSON.parse(cartSizes);
      if (!Array.isArray(sizesFromCart)) return;

      const validSizes = sizesFromCart
        .filter((item) => item?.size && Number(item.quantity) > 0)
        .map((item, index) => ({
          id: `${Date.now()}-${index}`,
          size: item.size,
      ...
      if (validSizes.length) setCottonSizes(validSizes);
    } catch (error) {
      console.error('Failed to restore cart sizes:', error);
    }
  }, [cartSizes, isCottonOurDesign]);

  useEffect(() => {
    // Revoke any outstanding object URLs on unmount to avoid memory leaks.
    return () => {
      [frontDesignPreview, backDesignPreview, logoPreview].forEach(url => {
        if (url?.startsWith('blob:')) URL.revokeObjectURL(url);
      });
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  // ── END Cotton Tee upload state ──────────────────────────────────
  ...
  useEffect(() => {
    ...
    const fetchCustomization = async () => {
      try {
        ...
        // =============================
        // CUSTOM COTTON TEE — hydrate UPLOAD_DESIGN customization for editing
        // =============================
        if (isCottonUploadDesign) {
          const fd = gf('frontDesign');
          const fdt = gf('frontDesignTransform');
          const bd = gf('backDesign');
          const bdt = gf('backDesignTransform');
          const lg = gf('logo');
          const lgt = gf('logoTransform');

          if (fd) setFrontDesignPreview(fd);
          if (fdt) { try { setFrontDesignTransform(JSON.parse(fdt)); } catch (e) { /* ignore malformed transform */ } }
          if (bd) setBackDesignPreview(bd);
          if (bdt) { try { setBackDesignTransform(JSON.parse(bdt)); } catch (e) { /* ignore malformed transform */ } }
          if (lg) setLogoPreview(lg);
          if (lgt) { try { setLogoTransform(JSON.parse(lgt)); } catch (e) { /* ignore malformed transform */ } }

          const sizesFromApi = (data.sizes || []).map(s => ({ id: Date.now().toString() + Math.random(), size: s.size, quantity: s.quantity || 1 }));
          if (sizesFromApi.length) setCottonSizes(sizesFromApi);

          return; // Cotton Tee customization does not use the Jersey hydration logic below.
        }
        // ── END Cotton Tee hydration ─────────────────────────────────
        ...
        if (patternId && patternFront && patternBack) {
          ...
          setSelectedPattern(null);
          setFrontImage(product?.cottonTee?.frontImage || product?.viewImages?.front || product?.images?.[0]);
          setBackImage(product?.cottonTee?.backImage || product?.viewImages?.back || product?.images?.[1]);
        }
        if (gf('jerseyColor')) setCustomizedFields(p => ({ ...p, jerseyColor: true }));
        if (gf('sleeveColor')) setCustomizedFields(p => ({ ...p, sleeveColor: true }));
        if (gf('collarColor')) setCustomizedFields(p => ({ ...p, collarColor: true }));
    ...
    fetchCustomization();
  }, [customizationId, product, isCottonUploadDesign]);
  ...
  // =============================
  // CUSTOM COTTON TEE — Add to Cart handlers
  // =============================
  const handleCottonOurDesignAddToCart = async () => {
    if (!product) { toast.error('Product not loaded'); return; }
    const sizesArray = cottonSizes
      .filter(s => s.size && (parseInt(s.quantity, 10) || 0) > 0)
      .map(s => ({ size: s.size, quantity: parseInt(s.quantity, 10) || 1 }));
    if (!sizesArray.length) { toast.error('Please select at least one valid size'); return; }

    try {
      // OUR_DESIGN is a ready-made design — no customization API call at all.
      await dispatch(addToCart({ productId: product._id || product.id, sizes: sizesArray })).unwrap();
      await dispatch(fetchCart());
      const totalQty = sizesArray.reduce((acc, s) => acc + s.quantity, 0);
      toast.success(`${product?.name} (${totalQty} items) added to cart!`);
    } catch (error) {
      toast.error(typeof error === 'string' ? error : error?.message || 'Failed to add to cart');
    }
  };

  const handleCottonUploadAddToCart = async () => {
    if (!product) { toast.error('Product not loaded'); return; }
    if (!frontDesignFileRef.current && !frontDesignPreview) { toast.error('Please upload front design'); return; }
    if (!backDesignFileRef.current && !backDesignPreview) { toast.error('Please upload back design'); return; }
    ...
    const customization = [
      { zoneKey: 'front', fieldName: 'frontDesign', value: '' },
      { zoneKey: 'front', fieldName: 'frontDesignTransform', value: JSON.stringify(frontDesignTransform) },
      { zoneKey: 'back', fieldName: 'backDesign', value: '' },
      { zoneKey: 'back', fieldName: 'backDesignTransform', value: JSON.stringify(backDesignTransform) },
    ];
    if (cottonLogoFileRef.current || logoPreview) {
      customization.push({ zoneKey: 'front', fieldName: 'logo', value: '' });
      customization.push({ zoneKey: 'front', fieldName: 'logoTransform', value: JSON.stringify(logoTransform) });
    }
    ...
    try {
      const res = await saveCustomizationAPI({
        productId: product._id || product.id,
        customizationId: '',
        customizationType: 'CUSTOM_COTTON_TEES',
        cottonTeeType: 'UPLOAD_DESIGN',
        customization,
        frontDesign: frontDesignFileRef.current,
        backDesign: backDesignFileRef.current,
        cottonLogo: cottonLogoFileRef.current,
      });
      const finalId = res?.data?._id || res?._id;
      if (!finalId) throw new Error('No customization ID returned');
  ...
  // Single dispatcher wired to the footer "Add to Cart" button.
  const handleAddToCart = () => {
    if (isCottonOurDesign) return handleCottonOurDesignAddToCart();
    if (isCottonUploadDesign) return handleCottonUploadAddToCart();
    return handleSaveDesign(); // NORMAL jersey — unchanged existing flow
  };
  // ── END Cotton Tee Add to Cart ───────────────────────────────────
  ...
  const reset = () => {
    ...
    // =============================
    // CUSTOM COTTON TEE — reset upload/transform state
    // =============================
    removeLogo(setFrontDesignPreview, frontDesignFileRef, frontDesignPreview);
    removeLogo(setBackDesignPreview, backDesignFileRef, backDesignPreview);
    removeLogo(setLogoPreview, cottonLogoFileRef, logoPreview);
    setFrontDesignTransform({ x: 50, y: 50, scale: 1, rotation: 0 });
    setBackDesignTransform({ x: 50, y: 50, scale: 1, rotation: 0 });
    setLogoTransform({ x: 20, y: 20, scale: 1, rotation: 0 });
    setCottonSizes([{ id: Date.now().toString(), size: 'M', quantity: 1 }]);
    // ── END Cotton Tee reset ─────────────────────────────────────────
  ...
  // =============================
  // CUSTOM COTTON TEE — size/quantity management (OUR_DESIGN & UPLOAD_DESIGN)
  // =============================
  const addCottonSize = () => {
    setCottonSizes(prev => [...prev, { id: Date.now().toString(), size: 'M', quantity: 1 }]);
  };
  const updateCottonSize = (id, field, value) => {
    setCottonSizes(prev => prev.map(item => item.id === id ? { ...item, [field]: value } : item));
  };
  const removeCottonSize = (id) => {
    setCottonSizes(prev => prev.filter(item => item.id !== id));
  };
  // ── END size/quantity management ─────────────────────────────────

  // =============================
  // CUSTOM COTTON TEE — tab renderers
  // =============================
  const renderCottonDesignTab = () => {
    const side = view; // reuse the existing Front/Back toggle on the main preview
    const isFrontSide = side === 'front';

    const designPreview = isFrontSide ? frontDesignPreview : backDesignPreview;
    const designTransform = isFrontSide ? frontDesignTransform : backDesignTransform;
    const setDesignPreview = isFrontSide ? setFrontDesignPreview : setBackDesignPreview;
    const setDesignTransform = isFrontSide ? setFrontDesignTransform : setBackDesignTransform;
    const designFileRef = isFrontSide ? frontDesignFileRef : backDesignFileRef;

    // =============================
    // CUSTOM COTTON TEE / UPDATE — shared zoom button style + clamp helper
    // =============================
    const ZOOM_MIN = 0.4;
    const ZOOM_MAX = 2.5;
    const ZOOM_STEP = 0.1;
    const zoomBtnStyle = {
      ...
      background: '#fff', color: '#003E9B', cursor: 'pointer',
      display: 'flex', alignItems: 'center', justifyContent: 'center',
    };
    const zoomIn = (setter) => setter(t => ({ ...t, scale: Math.min(ZOOM_MAX, +((t.scale || 1) + ZOOM_STEP).toFixed(2)) }));
    const zoomOut = (setter) => setter(t => ({ ...t, scale: Math.max(ZOOM_MIN, +((t.scale || 1) - ZOOM_STEP).toFixed(2)) }));

    return (
      <>
        <div style={{ padding: '10px 14px', background: '#EFF6FF', borderRadius: 10, border: '1px solid #BFDBFE', marginBottom: 10 }}>
          <div style={{ fontSize: 10, color: '#1D4ED8', fontWeight: 600 }}>
            💡 Your design auto-fits the print area. Drag it on the preview to move it, use zoom to resize — use the <b>FRONT / BACK</b> toggle to switch sides.
          </div>
        </div>

        <Section title={`${side === 'front' ? 'Front' : 'Back'} Design (Required)`} open={true} onToggle={() => {}}>
          <UploadSlot
            label={`${side === 'front' ? 'Front' : 'Back'} A4 Design`}
            hint="PNG/JPG image — auto-sized to the print area, drag on the preview to position it"
            state={designPreview}
            setter={setDesignPreview}
            fileRef={designFileRef}
            uid={`up-cotton-${side}-design`}
            onRemove={removeLogo}
          ...
          {designPreview && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 6 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <button onClick={() => zoomOut(setDesignTransform)} style={zoomBtnStyle} title="Zoom out"><ZoomOut size={14} /></button>
                  <span style={{ fontSize: 10, fontWeight: 700, color: '#334155', minWidth: 36, textAlign: 'center' }}>{Math.round((designTransform.scale || 1) * 100)}%</span>
                  <button onClick={() => zoomIn(setDesignTransform)} style={zoomBtnStyle} title="Zoom in"><ZoomIn size={14} /></button>
                </div>
              </div>
              <button onClick={() => setDesignTransform(t => ({ ...t, x: 50, y: 50, scale: 1 }))}
                style={{ fontSize: 10, fontWeight: 700, color: '#003E9B', background: 'rgba(0,62,155,0.08)', border: '1px solid rgba(0,62,155,0.25)', borderRadius: 8, padding: '9px', cursor: 'pointer', width: '100%' }}>
                ↺ Center Design
              </button>
            </div>
          )}
        ...
        <Section title="Logo (Optional)" open={true} onToggle={() => {}}>
          <UploadSlot
            label="Logo"
            hint="Optional — appears on the front only, drag on the preview to position it"
            state={logoPreview}
            setter={setLogoPreview}
            fileRef={cottonLogoFileRef}
            uid="up-cotton-logo"
            onRemove={removeLogo}
            onUpload={handleLogoUpload}
          />
          {logoPreview && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 6 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
                <span style={{ fontSize: 10, fontWeight: 700, color: '#64748B' }}>Zoom</span>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <button onClick={() => zoomOut(setLogoTransform)} style={zoomBtnStyle} title="Zoom out"><ZoomOut size={14} /></button>
                  <span style={{ fontSize: 10, fontWeight: 700, color: '#334155', minWidth: 36, textAlign: 'center' }}>{Math.round((logoTransform.scale || 1) * 100)}%</span>
                  <button onClick={() => zoomIn(setLogoTransform)} style={zoomBtnStyle} title="Zoom in"><ZoomIn size={14} /></button>
                </div>
              </div>
              <button onClick={() => setLogoTransform(t => ({ ...t, x: 20, y: 20, scale: 1 }))}
                style={{ fontSize: 10, fontWeight: 700, color: '#003E9B', background: 'rgba(0,62,155,0.08)', border: '1px solid rgba(0,62,155,0.25)', borderRadius: 8, padding: '9px', cursor: 'pointer', width: '100%' }}>
                ↺ Reset Position
              </button>
            </div>
  ...
  const renderCottonOrderTab = () => {
    const totalQty = cottonSizes.reduce((acc, s) => acc + (parseInt(s.quantity, 10) || 0), 0);
    ...
    return (
      <>
        <div style={{ padding: '10px 12px', background: '#F0FDF4', borderRadius: 10, marginBottom: 12, fontSize: 11, color: '#166534', fontWeight: 600 }}>
          {isCottonUploadDesign ? 'Design ready! Choose your sizes and complete your order.' : 'Choose your sizes and complete your order.'}
        </div>

        <Section title="Sizes & Quantity" open={true} onToggle={() => {}}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 12 }}>
            {cottonSizes.map((row) => (
              <div key={row.id} style={{ display: 'flex', gap: 8, alignItems: 'center', padding: '10px', border: '1px solid #E2E8F0', borderRadius: '10px', background: '#FAFAFA' }}>
                <select value={row.size} onChange={(e) => updateCottonSize(row.id, 'size', e.target.value)} style={{ flex: 1, padding: '8px', borderRadius: '6px', border: '1px solid #CBD5E1', fontSize: 11, outline: 'none', background: '#fff', color: '#000', fontWeight: 600 }}>
                  {SIZES.map(s => <option key={s} value={s}>{s}</option>)}
                </select>
                <input type="number" min="1" value={row.quantity} onChange={(e) => updateCottonSize(row.id, 'quantity', Math.max(1, parseInt(e.target.value, 10) || 1))} style={{ width: 64, padding: '8px', borderRadius: '6px', border: '1px solid #CBD5E1', fontSize: 11, outline: 'none', textAlign: 'center', color: '#000', fontWeight: 600 }} />
                {cottonSizes.length > 1 && (
                  <button onClick={() => removeCottonSize(row.id)} style={{ padding: '6px', background: '#FEE2E2', color: '#EF4444', border: '1px solid #FECACA', borderRadius: '6px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><X size={14} /></button>
                )}
              </div>
            ))}
            <button onClick={addCottonSize} style={{ padding: '10px', background: 'rgba(0,62,155,0.05)', color: '#003E9B', border: '1px dashed #003E9B', borderRadius: '10px', fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>+ Add Size</button>
          </div>
        </Section>
        ...
        <div style={{ border: '1px solid rgba(0,62,155,0.2)', borderRadius: 12, background: 'rgba(0,62,155,0.03)', padding: 16, marginBottom: 14 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 7 }}>
            <span style={{ fontSize: 10, color: '#64748B' }}>Sizes</span>
            <span style={{ fontSize: 11, fontWeight: 600, color: '#1E293B' }}>{cottonSizes.map(s => `${s.size} × ${s.quantity}`).join(', ') || '—'}</span>
          </div>
          <div style={{ borderTop: '1px solid rgba(0,62,155,0.12)', marginTop: 10, paddingTop: 10, display: 'flex', justifyContent: 'space-between', alignItems: 'center',color: '#64748B'}}>
            <span style={{ fontSize: 12, fontWeight: 700}}>Total ({totalQty} units)</span>
            <span style={{ fontSize: 22, fontWeight: 900, color: '#003E9B' }}>₹{(unitPrice * totalQty).toLocaleString()}</span>
          </div>
        </div>
      </>
    );
        ...
        <Section title="Players & Sizes" open={nameOpen} onToggle={() => setNameOpen(v => !v)}>
          ...
          {showName && (
            <>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 12 }}>
                {roster.map((player, idx) => (
                  <div key={player.id} style={{ display: 'flex', gap: 6, alignItems: 'center', padding: '8px 10px', border: '1px solid #E2E8F0', borderRadius: '10px', background: '#FAFAFA', width: '100%', boxSizing: 'border-box', minWidth: 0 }}>
                    <select value={player.size} onChange={(e) => updateRosterItem(player.id, 'size', e.target.value)} style={{ padding: '8px 4px', borderRadius: '6px', border: '1px solid #CBD5E1', fontSize: 11, outline: 'none', background: '#fff', color: '#000', fontWeight: 600, width: 50, flexShrink: 0 }}>
                      {SIZES.map(s => <option key={s} value={s}>{s}</option>)}
                    </select>
                    <select value={player.sleeve || 'Half'} onChange={(e) => updateRosterItem(player.id, 'sleeve', e.target.value)} style={{ padding: '8px 4px', borderRadius: '6px', border: '1px solid #CBD5E1', fontSize: 11, outline: 'none', background: '#fff', color: '#000', fontWeight: 600, width: 56, flexShrink: 0 }}>
                      <option value="Half">Half</option>
                      <option value="Full">Full</option>
                    </select>
                    <input type="text" placeholder="Name" value={player.name} onChange={(e) => {
                      updateRosterItem(player.id, 'name', e.target.value);
                      if (idx === 0) handlePlayerNameChange(e.target.value);
                    }} style={{ flex: 1, padding: '8px 6px', borderRadius: '6px', border: '1px solid #CBD5E1', fontSize: 11, outline: 'none', minWidth: 45, color: '#000', fontWeight: 600, boxSizing: 'border-box' }} />
                    <input type="text" placeholder="No." value={player.number} onChange={(e) => {
                      updateRosterItem(player.id, 'number', e.target.value);
                      if (idx === 0) handlePlayerNumberChange(e.target.value);
                    }} maxLength={2} style={{ width: 36, flexShrink: 0, padding: '8px 2px', borderRadius: '6px', border: '1px solid #CBD5E1', fontSize: 11, outline: 'none', textAlign: 'center', color: '#000', fontWeight: 600, boxSizing: 'border-box' }} />
                    {roster.length > 1 && (
                      <button onClick={() => removeRosterItem(player.id)} style={{ padding: '6px', background: '#FEE2E2', color: '#EF4444', border: '1px solid #FECACA', borderRadius: '6px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><X size={14} /></button>
                    )}
                  </div>
                ))}
                <button onClick={addRosterItem} style={{ padding: '10px', background: 'rgba(0,62,155,0.05)', color: '#003E9B', border: '1px dashed #003E9B', borderRadius: '10px', fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>+ Add Player</button>
  ...
  const panelInner = (
    <>
      ...
      <div style={{ padding: '12px 14px', borderTop: '1px solid #E8ECF0', background: '#fff', flexShrink: 0 }}>
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={reset} style={{ flex: 1, padding: '11px', borderRadius: 10, border: '1px solid #E2E8F0', background: '#fff', cursor: 'pointer', fontSize: 12, fontWeight: 700, color: '#64748B' }}>Reset</button>
          <button onClick={handleAddToCart} style={{ flex: 2, padding: '11px', borderRadius: 10, border: 'none', cursor: 'pointer', fontSize: 13, fontWeight: 800, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, background: 'linear-gradient(135deg,#0EA5E9,#0284C7,#1E3A8A)', color: '#fff' }}>
            <Save size={16} /> Add to Cart
          </button>
        </div>
      </div>
      ...
      >
        ...
        {viewMode === 'product' && (
          ...
          >
            {isCottonUploadDesign ? (
              // =============================
              // CUSTOM COTTON TEE: interactive design/logo stage replaces the
              // plain <img> + Canvas2DOverlay pairing used by Jersey products.
              // =============================
              <CottonDesignStage
                baseImage={view === 'front' ? frontImage : backImage}
                view={view}
                designPreview={view === 'front' ? frontDesignPreview : backDesignPreview}
                designTransform={view === 'front' ? frontDesignTransform : backDesignTransform}
                onDesignChange={view === 'front' ? setFrontDesignTransform : setBackDesignTransform}
                logoPreview={logoPreview}
                logoTransform={logoTransform}
                onLogoChange={setLogoTransform}
              />
            ) : (
              <>
                <img
