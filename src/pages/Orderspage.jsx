...
export default function Orderspage() {
  ...
  const handleViewOrder = async (order) => {
    ...
    try {
      ...
      const fresh = apiOrders.find((o) => o.orderId === order.id || o._id === order.backendId);
      if (fresh) {
        setSelectedViewOrder((prev) => ({
          ...prev,
          products: fresh?.orderDetails?.products || [],
          fullData: fresh,
        }));
      }
    } catch (_) {}
  ...
  const updateOrder = async (payload) => {
    try {
      await orderService.editOrder(selectedEditOrder._id, payload);
  ...
  const fetchOrders = async () => {
    try {
      ...
      const formattedOrders = apiOrders.map((order) => ({
        ...
        customer: order?.userDetails?.name || "Guest",
        ...
        deliveryDays: order.deliveryDays || "N/A",
          ...
          {TABS.map((tab) => (
            <button
              ...
              style={{
                padding: "10px 20px",
                background:
                  activeTab === tab.value ? activeTabBg : "transparent",
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          ...
          <tbody>
            {currentOrders.map((order, index) => {
              const status =
                STATUS_TYPE[order.orderStatus] || STATUS_TYPE.Pending;
                ...
                >
                  ...
                  <td style={{ padding: "16px", minWidth: "250px" }}>
                    {order.products.map((product, idx) => (
                      ...
                      >
                        <img
                          src={product.frontImage || product.viewImages?.front}
                        ...
                        <div>
                          ...
                          {(!product.sizes || product.sizes.length === 0) &&
                            ...
                            >
                              {(product.size ||
                                product.customization?.find((c) =>
                                  c.fieldName?.toLowerCase().includes("size"),
                                )?.value) && (
                                <span
                                  style={{
                                    fontSize: "10px",
                              ...
                              {(product.sleeve ||
                                product.customization?.find((c) =>
                                  c.fieldName?.toLowerCase().includes("sleeve"),
                                )?.value) && (
                                <span
                                  style={{
                                    fontSize: "10px",
                              ...
                              {product.customization?.find(
                                (c) =>
                                  c.fieldName?.toLowerCase() === "playernumber",
                              )?.value && (
                                <span
                                  style={{
                                    fontSize: "10px",
                  ...
                  <td style={{ padding: "16px", textAlign: "center" }}>
                    ...
                    >
                      <IoWallet size={12} />₹{order.total.toLocaleString()}
                    </span>
                  </td>
                  <td style={{ padding: "16px", textAlign: "center" }}>
                    <div
                ...
                return (
                  <button
                    ...
                    style={{
                      padding: "8px 14px",
                      background:
                        currentPage === pageNum ? primaryGradient : cardBg,
                      border: `1px solid ${borderColor}`,
                      borderRadius: "8px",
                      cursor: "pointer",
